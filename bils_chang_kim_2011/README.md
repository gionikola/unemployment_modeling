...
The following is a plot of an employed worker's asset policy function.
The first concerning aspect of this plot is the fact that the policy function seems to be (almost) identicaly across all match productivity values.
The second concerning property of the plot is the strange break between the asset values of 25 and 50 -- you will notice this to be a recurring theme in later figures as well.
From some of my diagnostics I've noticed that this issue is independent of reasonable initial value function and wage schedule guesses, and seems to stem from the workers' value function iteration portion of algorithm.
Making the asset grid less coarse seems to address this problem to an extent -- but as mentioned before, doing so would make the run-time much greater.

![](../bils_chang_kim_2011/figures/fig1.png)

The following plot is that of an unemployed worker's asset policy function.
This function seems more natural, although it also begins to act strangely below the asset level between 25 and 50.
This issue is likely bleeding over from the employed worker's value function, since it appears in the unemployed worker's value function.

![](../bils_chang_kim_2011/figures/fig2.png)

The following replicates Panel C. of Figure 1 in the paper, which presents a plot of the value of a firm-worker match
The shape of the function seems familiar above the notorious breaking point in the asset grid, but is completely incorrect below it.
Once again this seems to be caused by shape of the worker's problem, which affects the matched firm value directly through the worker's value functions. 

![](../bils_chang_kim_2011/figures/fig3.png)

The following plot replicates Panel A of Figure 1 in the paper. 
Note the recurring issue. 

![](../bils_chang_kim_2011/figures/fig4.png)

The following plot replicates Panel B of Figure 1 in the paper.
The familiar issue no longer seems to crop up, and the plotteed function quite closely matches its counterpart in the article.
It seems to be the case that the issue affects the values of employment and unemployment proportionally, so that their difference cancels out the abnormality.
A concern that is still present, however, is the fact that the plotted function is almost identically across the match productivity grid.

![](../bils_chang_kim_2011/figures/fig5.png)

The following plot replicates Panel A of Figure 2 of the paper.
We can see that the distributions for both employed and unemployed workers seems to be concentrated at the very top of the asset grid, when if fact we should be seeing the opposite.

![](../bils_chang_kim_2011/figures/fig6.png)

Lastly, the following plot replicates Panel B of Figure 2 of the paper, and it seems to do so with relatively success.
This probably stems once again from the fact that the underlying issue affecting the other figures has proportional effets on the workers' employment and unemployment values, which gets cancelled out when the two value functions are compared. 
